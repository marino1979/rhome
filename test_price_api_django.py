#!/usr/bin/env python
"""
Test per verificare che l'API di calcolo prezzi funzioni correttamente usando Django.
"""
...
def test_price_api_django():
    """Test dell'API di calcolo prezzi usando Django test client."""
    print("TEST API CALCOLO PREZZI (Django)...")
    ...
    try:
        ...
        if response.status_code != 200:
            print(f"   ERRORE: Status code non 200. Response: {response.content.decode()}")
            return False
        ...
        response = client.post(
            '/api/calculate-price/',
            data=json.dumps({'listing_id': listing.id}),
            content_type='application/json'
        )
